...
BandwidthVoice.Configuration.basicAuthUserName = process.env.BANDWIDTH_USERNAME;
BandwidthVoice.Configuration.basicAuthPassword = process.env.BANDWIDTH_PASSWORD;
...
app.get("/startPSTNCall", async (req, res) => {
  try {
    ...
    console.log(
        `Call created: ${callId} from: ${process.env.BANDWIDTH_PHONE_NUMBER} to: ${process.env.OUTBOUND_PHONE_NUMBER}`
    );
...
async function initiateCallToPSTN(account_id, from_number, to_number) {
  ...
  var body = new BandwidthVoice.ApiCreateCallRequest({
    from: from_number,
    to: to_number,
    applicationId: process.env.BANDWIDTH_VOICE_APPLICATION_ID,
    answerUrl: process.env.BASE_URL + "callAnswered",
    answerMethod: "POST",
    callTimeout: "30",
  });
