...
class Settings {
    ...
    var address: String = {
        guard let path = Bundle.main.path(forResource: "Config", ofType: "plist"), let data = try? Data(contentsOf: URL(fileURLWithPath: path)) else {
            fatalError("Failed to load configuration property list. Did you remember to create a Config.plist file?")
        }
        ...
        guard let address = configuration["Address"] else {
            fatalError("Failed to load address. Did you remember to add an Address key with the value of your application server address?")
        }
